...
export default function ServiceDetailPage({ params }) {
  ...
  return (
    <div className={`w-full my-28 pt-10 px-1 transition-colors duration-300 ${darkMode ? ' text-white' : ' text-white'}`}>
      {!isDesktop && (
        <h1 className={`text-[32px] xs:text-4xl md:text-6xl text-center tracking-widest font-bold uppercase mt-10 text-[#395299] leading-tight ${darkMode ? 'text-[#fbcc03]' : 'text-[#395299]'}`}>
          {title}
        </h1>
      )}
            ...
            <div className="flex flex-col h-full justify-center items-start">
              ...
              >
                {details.map((sec, i) => (
                  ...
                  >
                    <p className={`lg:text-[20px] xl:text-[22px] 3xl:text-[28px] tracking-widest ${darkMode ? 'text-gray-300' : 'text-black font-medium'}`}>
                      {sec.description}
                    </p>
                  </div>
                ))}
        ...
        <div className="w-full lg:w-1/2">
          {isDesktop ? (
            details.map((sec, i) => (
              ...
              >
                <div className={`rounded-3xl p-8 w-full max-w-[500px] xl:max-w-[650px] 2xl:max-w-[700px] 3xl:max-w-[900px] shadow-lg ${darkMode ? 'bg-black' : 'bg-black border border-gray-200'}`}>
